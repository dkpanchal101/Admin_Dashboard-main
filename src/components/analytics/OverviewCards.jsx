import { motion } from "framer-motion";
import { useFilters } from "../../context/FilterContext";
import { DollarSign, Users, ShoppingBag, Eye, ArrowDownRight, ArrowUpRight } from "lucide-react";

const overviewData = [
	{ name: "Revenue", value: "$1,234,567", change: 12.5, icon: DollarSign },
	{ name: "Users", value: "45,678", change: 8.3, icon: Users },
	{ name: "Orders", value: "9,876", change: -3.2, icon: ShoppingBag },
	{ name: "Page Views", value: "1,234,567", change: 15.7, icon: Eye },
];

const OverviewCards = () => {
    const { selectedCategory, selectedChannel, clearFilters } = useFilters();
    return (
        <div className='grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 mb-8'>
			{overviewData.map((item, index) => (
				<motion.div
					key={item.name}
                    className='card p-6'
					initial={{ opacity: 0, y: 20 }}
					animate={{ opacity: 1, y: 0 }}
					transition={{ delay: index * 0.1 }}
				>
					<div className='flex items-center justify-between'>
						<div>
                        <h3 className='text-sm font-medium muted'>{item.name}</h3>
                        <p className='mt-1 text-xl font-semibold' style={{ color: "rgb(var(--text))" }}>{item.value}</p>
						</div>

						<div
							className={`
              p-3 rounded-full bg-opacity-20 ${item.change >= 0 ? "bg-green-500" : "bg-red-500"}
              `}
						>
							<item.icon className={`size-6  ${item.change >= 0 ? "text-green-500" : "text-red-500"}`} />
						</div>
					</div>
                <div
                    className={`
              mt-4 flex items-center ${item.change >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}
            `}
                >
						{item.change >= 0 ? <ArrowUpRight size='20' /> : <ArrowDownRight size='20' />}
						<span className='ml-1 text-sm font-medium'>{Math.abs(item.change)}%</span>
                    <span className='ml-2 text-sm muted'>vs last period</span>
					</div>
				</motion.div>
			))}
            {(selectedCategory || selectedChannel) && (
                <div className='sm:col-span-2 lg:col-span-4 flex items-center justify-between card p-3'>
                    <div className='text-sm'>
                        Filters: {selectedCategory ? `Category = ${selectedCategory}` : ''} {selectedChannel ? `Channel = ${selectedChannel}` : ''}
                    </div>
                    <button className='btn' onClick={clearFilters}>Clear Filters</button>
                </div>
            )}
		</div>
	);
};
export default OverviewCards;